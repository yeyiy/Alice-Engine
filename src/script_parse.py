...
def parse_script(string: str):
    ...
    def _split(t: str, x: str):
        ...
        for j in range(len(t)):
            ...
            if num == 0 and t[j:j + len(x)] == x:
                result.append(t[start:j])  # 当不在嵌套的节点内且遇到要分割的字符串时，将分割的字符串加入列表
                start = j + len(x)

        result.append(t[start:])  # 将最后一个分割的字符串加入列表，以免被忽略
    ...
    def parse_point(point):
        ...
        # 解析代码调用
        for s in range(int(point.count("@") / 2) + 1):
            code_start = None  # 调用代码开始位置
            n = 0  # 嵌套层数
            ...
            for k in range(len(point)):
                ...
                if n == 0 and point[k] == "@":  # 遇到“@”且没处在嵌套内标记代码开始或结束，避免重复调用
                    if code_start is None:
                        code_start = k  # 标记代码调用开始
                        continue

                    else:
                        point = point[:code_start] + runner.run_code(point[code_start + 1:k]) + point[k + 1:]  # 将代码调用替换为执行结果
                        break
